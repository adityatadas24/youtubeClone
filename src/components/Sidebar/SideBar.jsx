                ...
                {sidebarLinks.map((category, catIndex) => (
                    <div key={catIndex}>
                        {category.categoryTitle && (
                            <h4 className="text-[15px] font-bold mb-2 ml-2 mt-4 dark:text-neutral-300">
                                {category.categoryTitle}
                            </h4>
                        )}
...
export const Link = ({ link, IconComponent }) => {
    return (
        <a
            href={link.url}
            className={`flex text-[13px]  font-semibold text-black items-center py-2.5 px-3 rounded-lg hover:bg-neutral-200 mb-1 whitespace-nowrap dark:text-neutral-300 dark:hover:bg-neutral-500`}
        >
